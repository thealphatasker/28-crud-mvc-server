...
function App() {
  ...
  const handleNewProductsFormChange = (e) => {
    const value =
      e.target.name === "id" || e.target.name === "price"
  ...
  async function saveProduct(e) {
    ...
    try {
      ...
      const existingProduct = products.find(
        (p) => p.id === newProductsForm.id.toString(),
      );
      if (existingProduct) {
        alert(
          `Product with ID ${newProductsForm.id} already exists! Please use a different ID.`,
        );
        return;
      }
  ...
  async function editProduct(e) {
    ...
    try {
      ...
      const updateData = {};
      if (newProductsForm.name) updateData.name = newProductsForm.name;
      if (newProductsForm.price) updateData.price = newProductsForm.price;
